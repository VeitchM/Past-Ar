...
export default function DevicesModal(props: {
  ...
  const devices: DeviceSerializable[] = useTypedSelector(
    (state) => state.ble.allDevices
  );
    ...
    <Modal isOpen={props.showModal} onClose={() => props.setShowModal(false)}>
      <Modal.Content maxWidth="400px">
        ...
        <FlatList
          data={devices}
          renderItem={({ item }) => (
            <DeviceItem item={item} onSelected={connectDevice} />
          )}
...
function DeviceItem(props: { item: DeviceSerializable; onSelected: any }) {
  return (
    <Button
      variant="unstyled"
