...
export default function ForSendingCalibrationsScreen({
  navigation,
}: PropsCalibrationForSending) {
  //Value represents id in database
  const [calibrations, setCalibrations] =
    useState<CalibrationsFromMeasurementsLocalDB[]>();
  ...
  const onChangeSendStatus = useCallback(
    (item: CalibrationsFromMeasurementsLocalDB) => {
      const sendStatus =
        item.sendStatus === SendStatus.NOT_SENT
      ...
      setSendStatus(
        ...
        pushNotification(
          "No se ha podido agregar a la cola de envio la calibracion: " +
            item.name,
          "error",
        );
  ...
  const Item = useCallback(
    (props: { item: CalibrationsFromMeasurementsLocalDB }) => {
      const { item } = props;
      return (
        <Pressable
          key={item.ID}
        ...
        >
          ...
          >
            ...
            <Checkbox
              ...
              accessibilityLabel="forUploading"
              colorScheme="blue"
              size="lg"
              icon={
                <Icon size="xl" as={<MaterialCommunityIcons name="upload" />} />
              }
