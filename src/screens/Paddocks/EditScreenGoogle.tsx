...
export default function EditScreenGoogle(props: Props) {
      ...
      >
        ...
        >
          ...
          {!openInfo ? (
            ...
            >
              {addingMode
                ? "Presiona el mapa para agregar vertices"
  ...
  const polygonOverlaps = (polygon: LatLng[], selectedPaddockUid?: number) => {
    ...
    return paddocks
      .filter(
        (paddock, index) =>
          paddock.ID !== selectedPaddockUid && paddock.vertices.length > 0
      )
      .some(({ vertices }) => {
  ...
  function OtherPolygons() {
    return (
      <>
        {paddocks
          .filter((p, i) => {
            return p.ID != paddockId && p.vertices.length > 0;
          })
          .map((paddock, index) => {
            return !!paddock ? (
              <View key={"P" + index}>
          ...
          return (
            <Marker
              ...
              pinColor={
                addingMode
                  ? "#2ecc71"
                  : index == currentPointId
                  ? "#f1c40f"
                  : "#3498db"
              }
              key={
                "M" + index + "-" + (index == currentPointId) + "-" + addingMode
              }
              onPress={() => {
                if (!addingMode) {
  ...
  function checkAndInsertPin(newCoords: LatLng, vertices?: LatLng[]) {
    ...
    if (!error && !!poly) {
      ...
      if (!vertices)
        setActionStack([
          ...actionStack,
          { actionType: "ADD", coord: newCoords },
        ]);
  ...
  function checkPinDrag(newCoords: LatLng, pinId: number) {
    ...
    if (pointsInsidePolygon(_vertices, paddocks, paddockId)) {
      ...
      //Check if the new polygon will have crossed sides
      let res = getPolygonWithPoint(
        polygonVertices.filter((p, i) => {
          return i != pinId;
        }),
        newCoords
      );
    ...
    if (!error && !!poly) {
      ...
      setActionStack([
        ...actionStack,
        { actionType: "MOVE", oldCoord: oldCoord, coord: newCoords },
      ]);
      setPolygonVertices(poly);
      return true;
    } else Toast.show({ description: error, bottom: 10 });
  ...
  function undoHandler() {
    if (addingMode && actionStack.length > 0) {
      ...
      let vertices = polygonVertices.filter((v, i) => {
        return (
          v.latitude != lastAction?.coord.latitude &&
          v.longitude != lastAction?.coord.longitude
        );
      });
