...
export default function FiltersScreen(props: Props) {
  ...
  useEffect(() => {
    ...
    if (filterState.filteredPaddock)
      setFilteredPaddock(filterState.filteredPaddock);
    if (filterState.filteredSector)
      setFilteredSector(filterState.filteredSector);
  ...
  const FilterButton = (props: any) => {
    ...
    return (
      <View
        ...
        borderRadius={20}
        margin={1}
        borderWidth={currentButton == label && showMeasurements ? 2 : 0}
        width={95}
        height={65}
      >
        <Button
      ...
      >
        ...
        <View height={30} flexDirection={"row"} justifyContent={"flex-start"}>
          ...
          <Switch
            ...
            onToggle={(state) => {
              setShowMeasurements(state);
              paddockSheet.current?.close();
              sectorSheet.current?.close();
            }}
          />
        </View>
        ...
        <Box flexWrap={"wrap"} flexDir={"row"} justifyContent={"space-evenly"}>
          <FilterButton
            label={"24 " + TS.t("hour_short")}
            onPress={() => {
              setFilterPeriod("days", 1);
            }}
          />
          <FilterButton
            label={"48 " + TS.t("hour_short")}
            onPress={() => {
              setFilterPeriod("days", 2);
            }}
          />
          <FilterButton
            label={"1 " + TS.t("week_short")}
            onPress={() => {
              setFilterPeriod("days", 7);
            }}
          />
          <FilterButton
            label={"1 " + TS.t("month_short")}
            onPress={() => {
              setFilterPeriod("months", 1);
            }}
          />
          <FilterButton
            label={"5 " + TS.t("month_short")}
            onPress={() => {
              setFilterPeriod("months", 5);
            }}
          />
          <FilterButton
            label={"1 " + TS.t("year_short")}
            onPress={() => {
              setFilterPeriod("years", 1);
            }}
          />
        </Box>
        ...
        <View flexDirection={"row"}>
          <Button
            ...
            isDisabled={!showMeasurements}
            onPress={() => {
              paddockSheet.current?.snapToIndex(0);
            }}
            endIcon={
              filteredPaddock >= 0 ? (
                <Icon as={FontAwesome5} name="check-circle" size="md" />
              ) : (
                <></>
              )
            }
          ...
          <Button
            ...
            isDisabled={!showMeasurements}
            onPress={() => {
              sectorSheet.current?.snapToIndex(0);
            }}
            endIcon={
              filteredSector >= 0 ? (
                <Icon as={FontAwesome5} name="check-circle" size="md" />
          ...
          onPress={() => {
            dispatch(
              updateFilter({
                ...
                until: until.getTime(),
                filteredPaddock:
                  filteredPaddock >= 0 ? filteredPaddock : undefined,
                filteredSector:
                  filteredSector >= 0 ? filteredSector : undefined,
                shortcutMapId: currentButton,
              }),
            );
        ...
        >
          <Heading
            ...
            color={"trueGray.500"}
          >
            {TS.t("filters_by_paddock").toUpperCase()}
          </Heading>
          <Divider />
          <BottomSheetFlatList
            ...
            renderItem={({ item, index }) => (
              <Box
                marginBottom={index >= paddockList.length - 1 ? 8 : 3}
                marginLeft={3}
                marginRight={3}
              >
                <TouchableHighlight
                ...
                >
                  <Box
                    ...
                    rounded={"lg"}
                    backgroundColor={
                      filteredPaddock == item.id ? "#525252" : "trueGray.100"
                    }
                    padding={3}
                  >
                    <Heading
                      color={filteredPaddock == item.id ? "#fff" : "#525252"}
                    >
                      {" "}
                      {item.name}{" "}
                    </Heading>
        ...
        >
          <Heading
            ...
            color={"trueGray.500"}
          >
            {TS.t("filters_by_sector").toUpperCase()}
          </Heading>
          <Divider />
          <BottomSheetFlatList
            style={{ paddingTop: 15, position: "relative" }}
            ...
            renderItem={({ item, index }) => (
              <Box
                marginBottom={index >= sectors.length - 1 ? 8 : 3}
                marginLeft={3}
                marginRight={3}
              >
                <TouchableHighlight
                ...
                >
                  <Box
                    ...
                    rounded={"lg"}
                    backgroundColor={
                      filteredSector == item.ID ? "#525252" : "trueGray.100"
                    }
                    padding={3}
                  >
                    <Heading
                      color={filteredSector == item.ID ? "#fff" : "#525252"}
                    >
                      {" "}
                      {FormatUtils.formatExtendedDate(
