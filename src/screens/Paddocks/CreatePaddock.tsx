...
export default function CreatePaddock(props: Props) {
    ...
    useEffect(() => {
        ...
        getLocation().then((value) => { if (value) setCurrentLocation(value.coords) })
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <View flexDir={'row'} paddingTop={2} justifyContent={'space-around'} width={'100%'}>
                        ...
                        <TouchableHighlight style={{ flex: 1, borderRadius: 16, marginLeft: 5 }} activeOpacity={0.8} onPress={() => { }}>
                            <Input
                                flex={1}
                                fontSize={'lg'}
                        ...
                        <IconButton
                            ...
                            marginLeft={3}
                            onPress={() => {
                                navigation.dispatch(CommonActions.navigate({ name: 'EditScreen', params: { coordinates: item, paddockId: paddockId, pointId: index } }))
                            }}
                            colorScheme='green'
                        ...
                        if (paddockId != undefined && paddockId >= 0) {
                            navigation.dispatch(
                                CommonActions.navigate({
                                    name: 'EditScreen',
                                    params: {
                                        coordinates: paddockList[paddockId].vertices.length > 0 ? paddockList[paddockId]?.vertices[0] : currentLocation,
                                        paddockId: paddockId,
                                        pointId: 0
                                    }
                                })
    ...
    return (
        <VStack bg='white' flex={1} alignItems='center' paddingTop={5}>
            ...
            <VerticesList />
            <HStack bottom={5} height={70} width={'100%'} justifyContent={'space-around'} paddingLeft={10} paddingRight={10} >
                <AddVerticesButton />
                <SavePaddockButton />
