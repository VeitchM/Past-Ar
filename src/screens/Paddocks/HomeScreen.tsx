...
import DownloadTilesButton from "./Components/DownloadTilesButton";
import IMapView from "./Components/MapViewInterface";
...
export default function PaddockScreen(props: Props) {
  ...
  const [region, setRegion] = useState<LatLng & { zoom: number }>({
    latitude: 0,
    longitude: 0,
    zoom: 12,
  });
  ...
  async function initializePaddockList() {
    ...
    resultPaddocks.forEach((value) => {
      ...
      if (
        ...
        paddocks = [...paddocks, paddockData];
    });
    const crossedPaddocks = await getCrossedPaddocks();
    setPaddocksFromBack(
      crossedPaddocks.map((r) => {
        return r.ID;
      })
    );
  ...
  const fetchLocation = async (change: boolean = true) => {
    ...
    let loc = await Promise.race<LocationObject | AsyncReturn>([
      getLocation(),
      Async.resolveAfter(Async.MID),
    ]);
      ...
      >
        ...
        >
          <Icon
            as={FontAwesome5}
            size={10}
            name={isLocationUpdating ? undefined : "compass"}
            color="coolGray.600"
          />
          <ActivityIndicator
            style={{ position: "absolute" }}
            animating={isLocationUpdating}
            size={"large"}
            color={"#4b5563"}
          />
        </TouchableOpacity>
        ...
        >
          ...
          >
            ...
            <Heading size={"md"} textAlign={"left"}>
              {`${TS.t("paddocks_info_latitude")}:     ${
                currentCoords.latitude + "\n" + TS.t("paddocks_info_longitude")
        ...
        <BottomSheetFlatList
          ...
          renderItem={({ item, index }) => (
            <BottomSheetItem
              ...
              onLocatePress={() => {
                changeRegion(
                  paddockList[index].vertices[0].latitude,
                  paddockList[index].vertices[0].longitude
                );
              ...
              onEditPress={() => {
                ...
                let newIndex = dispatch(addPaddock({ data: paddockData }))
                  .payload.index;
                sheetRef.current?.snapToIndex(0);
