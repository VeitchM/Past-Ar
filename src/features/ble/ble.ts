...
import { setConnectedDevice, addDevice, resetDevices, setTryingToConnect } from "./bleSlice";
...
const onAnomalousDisconnection = (deviceId: string) => {
  ...
  onAnomalousDisconnectionSubscription = bleManager.onDeviceDisconnected(deviceId, (error, device) => {
    ...
    console.error('On device disconnected onAnomalousDisconnection callback was called', new Date());
    ...
    if (store.getState().ble.connectionState != 'disconnected') {
...
const disconnectFromDevice = async () => {
  ...
  console.log('Try disconnect: ', connectedDevice);
